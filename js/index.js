...
const streamURL = "http://192.168.137.200:81/stream"; // Đảm bảo rằng stream URL đúng

function updateStream() {
  const img = document.getElementById("esp32cam-stream"); // Đảm bảo ID img đúng trên tất cả các trang
  if (img) {
...
function handleStreamError() {
  ...
  if (img) {
    ...
    // Hiển thị ảnh lỗi
    img.src = "error-image.png"; // Đảm bảo bạn có file error-image.png trong thư mục phù hợp

    // Thử kết nối lại sau 3 giây
    setTimeout(updateStream, 3000); // Retry after 3 seconds
  }
}

// 🔹 Cập nhật hình ảnh mới mỗi giây
setInterval(updateStream, 1000); // Updates the image every second

// 🔹 Tải ảnh lần đầu tiên khi trang mở
document.addEventListener("DOMContentLoaded", () => {
  updateStream(); // Cập nhật stream ngay khi DOM đã tải xong
  const img = document.getElementById("esp32cam-stream");
  if (img) {
    img.onerror = handleStreamError; // Xử lý lỗi nếu không thể tải stream
  }
});
...
function updateSensorStatus(sensors) {
    sensors.forEach((sensor) => {
        if (sensor.location === "Office") {
            document.querySelector(".office .humidity").innerText = sensor.humidity + "%";
            document.querySelector(".office .temperature").innerText = sensor.temperature + "°C";
            document.querySelector(".kitchen .light_lux").innerText = sensor.light + " Lux";
            document.querySelector(".tank .gas").innerText = sensor.gas + " ppm";
        }
        if (sensor.location === "Kitchen") {
            document.querySelector(".kitchen .light_lux").innerText = sensor.light + " Lux";
        }
        if (sensor.location === "Tank") {
            document.querySelector(".tank .gas").innerText = sensor.gas + " ppm";
        }
    });
}
